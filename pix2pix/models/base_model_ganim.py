...
class BaseModel_ganim:
    """docstring for BaseModel"""
    def __init__(self):
        super(BaseModel_ganim, self).__init__()
        self.name = "Base"
    ...
    def setup(self):
        mode = "test" #MODE SET FOR TEST
        print("%s with Model [%s]" % (mode.capitalize(), self.name))
        if self.is_train:
            "SET FOR TRAIN!!"
            self.set_train()
...
#REMOVED THE OPTION FOR TRAIN!
    ...
    def load_ckpt(self, epoch, models_name):
        # print(models_name)
        #YOU CAN PUT THIS PATH FOR LOADING THE FILE IN THE ARGPARSER!!
        ckpt_dir = "/home/ubuntu/KRB_Projects/wav2exp/Mark_2_backup/fined_tuned_ganimation_with_no_roi"
        for name in models_name:
            if isinstance(name, str):
