...
#FEED BATCH FUNCTION IS ALTERED SO THAT IT CAN TAKE ONLY SRC AND TAR IMAGES. Line 40. (Thank you!)
class GANimationModel(BaseModel_ganim):
    ...
    def initialize(self):
        super(GANimationModel, self).initialize()
        ...
        if self.is_train:
            self.net_dis = model_utils.define_splitD(3, 17, 128, 64, 
                    norm='instance', init_type='normal', init_gain=0.02, gpu_ids=[0])
            self.models_name.append('dis')

        self.load_ckpt(5) #Looad epoch
    ...
    def feed_batch(self, src_img, tar_aus):
        self.src_img = src_img.to(self.device)
        self.tar_aus = tar_aus.type(torch.FloatTensor).to(self.device)
        #TENSORS ARE ASSIGNED AS CLASS VARIABLES
        #NOT REQUIRED 'TRAIN' FOR THIS TASK.
        # if self.is_train:
